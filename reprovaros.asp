...
<body bgcolor=#F0FFF0>
<form method=post action=reprovaros.asp target=_self>
...
<%       if request.form("confbusca") = "" then 'Verifica para buscar OS aberta e não aprovadas
          set bdapros = conexao.execute("select nros, codfunc from oscon where codsitua = 13 or codsitua = 1 order by nros")
          ...
          if request.form("confgr") = "" then
           set bdapros = conexao.execute("select codfunc, nros, nipatr, local, servso, dtabe, dtapr, dtdir from oscon where nros =" & request.form("nrosapfr"))%>
...
<%         if bdapros("dtapr") <> 0 then %>       
            <tr>
             <td><font face=Arial size=2>Data aprovação:</font></td>
             <td><font face=Arial size=2><%=bdapros("dtapr")%></font></td>
            </tr>
<%         end if  
           if bdapros("dtdir") <> 0 then %>
            <tr>
             <td><font face=Arial size=2>Data direcionado:</font></td>
             <td><font face=Arial size=2><%=bdapros("dtdir")%></font></td>
            </tr>
<%         end if %>
            <tr>
             <td><font face=Arial size=2>Direcionar:</font></td>
             <td>
              <select name=direcfr>
...
<%         set bdapros = nothing   
           ...
           if request.form("obsfr") = "" then
             response.write "O campo observação não pode ser ser em branco quando a OS é reprovada<br><a href=javascript:history.go(-1)>Voltar</a>"
           else
           if len(request.form("obsfr")) < 225 then
            if request.form("direcfr") = "2" then
             ...
             response.write "A OS " & request.form("nrosapfr") & " foi direcionada para o docente!<br><a href=reprovaros.asp>Reprovar outra OS</a>"
             set bdapros = nothing 
            end if
            if request.form("direcfr") = "4" then
             ...
             response.write "A OS " & request.form("nrosapfr") & " foi reprovada com sucesso!<br><a href=reprovaros.asp>Reprovar outra OS</a>"
             set bdapros = nothing 
            end if
...
'Envia um email para o coordenador apos a abertura da OS

set buscacodnucleo = conexao.execute("select codnucl, email from func where codfunc ='" & request.form("solicfr") & "'")

set buscaemail = conexao.execute("select email from func where codsitua = 7 and codcarg = 2 and codnucl = " & buscacodnucleo("codnucl"))

assunto = "A OS " & request.form("nrosapfr") & " foi reprovada por " & session("func")
...
correio.send "suporteinf116@sp.senai.br (Sistema de OS)", buscacodnucleo("email"), assunto, corpoemail, 2
